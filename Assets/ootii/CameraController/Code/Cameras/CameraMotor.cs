        ...
        public virtual Vector3 GetFocusPosition(Quaternion rCameraRotation)
        {
            ...
            if (lAnchorTransform != null)
            {
                ...
                if (RigController.IsCollisionsEnabled && _IsCollisionEnabled && lAnchorOffset.sqrMagnitude > 0f)
                {
                    ...
                    if (RaycastExt.SafeSphereCast(lAnchorTransform.position + (lAnchorDirection * lDistanceOffset), lAnchorDirection, RigController.CollisionRadius * 1.1f, out lFocusHit, lAnchorDistance - lDistanceOffset, RigController.CollisionLayers, lAnchorTransform))
                    {
                    ...
                    {
                        Graphics.GraphicsManager.DrawCapsule(lAnchorTransform.position + (lAnchorDirection * lDistanceOffset), lAnchorTransform.position + lAnchorOffset, RigController.CollisionRadius * 1.1f, Color.green);
                    }
#endif

                }

                lNewFocusPosition = lNewFocusPosition + lAnchorTransform.position + lAnchorOffset + (lAnchorTransform.up * _Offset.y);
                ...
                if (RigController.IsCollisionsEnabled && _IsCollisionEnabled && Offset.sqrMagnitude > 0f)
                {
                    ...
                    RaycastHit lFocusHit;
                    if (RaycastExt.SafeSphereCast(lAnchorPosition, lFocusDirection, RigController.CollisionRadius * 1.1f, out lFocusHit, lFocusDistance, RigController.CollisionLayers, lAnchorTransform))
                    {
                    ...
                    if (RigController.EditorShowDebug)
                    {
                        Graphics.GraphicsManager.DrawCapsule(lAnchorPosition, lNewFocusPosition, RigController.CollisionRadius * 1.1f, Color.cyan);
                    }
#endif
