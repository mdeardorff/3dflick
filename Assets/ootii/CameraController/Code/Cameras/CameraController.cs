    ...
    public class CameraController : BaseCameraRig
    {
        ...
        public void EnableMotor<T>(bool rEnable, string rName = null) where T : CameraMotor
        {
            ...
            for (int i = 0; i < Motors.Count; i++)
            {
                ...
                if (lType == lBaseType || lType.GetTypeInfo().IsAssignableFrom(typeof(T).GetTypeInfo()))
#else
                if (lType == lBaseType || lType.IsSubclassOf(typeof(T)))
#endif
                {
        ...
        public override void SetTargetYawPitch(float rYaw, float rPitch, float rSpeed = -1f, bool rAutoClearTarget = true)
        {
            ...
            {
                YawPitchMotor lMotor = Motors[i] as YawPitchMotor;
                if (lMotor != null) { lMotor.SetTargetYawPitch(rYaw, rPitch, rSpeed, rAutoClearTarget); }
            }
        }
        ...
        public override void RigLateUpdate(float rDeltaTime, int rUpdateIndex)
        {
            ...
            for (int i = 0; i < Motors.Count; i++)
            {
                ...
                if (Motors[i].TestActivate(lActiveMotor))
                {
        ...
        private void UpdateZoom(float rDeltaTime)
        {
            ...
            if (Mathf.Abs(mTargetFOV - _Camera.fieldOfView) > 0.001f)
            {
        ...
        private void UpdateCollisions(float rDeltaTime, ref CameraTransform rTransform)
        {
            ...
            if (_IsCollisionsEnabled && (lActiveMotor == null || lActiveMotor.IsCollisionEnabled))
            {
                ...
                {
                    float lDistance = lViewHit.distance;
                    ...
                    if (lDistance == 0f)
                    {
                        ...
                        {
                            lDistance = lViewHit.distance;
                        }
                    }

                    if (lDistance > 0f)
                    {
                        float lDelta = lDistance - lOldDistance;
                        if (lDelta <= lFrameMovement)
                        {
                            mHadCollided = true;
                            lIsColliding = true;
                            rTransform.Position = lNewFocusPosition + (lNewDirection * Mathf.Max(lDistance, _MinCollisionDistance));
                        }
                    }
                }
                ...
                if (EditorShowDebug)
                {
                    Graphics.GraphicsManager.DrawCapsule(lNewFocusPosition, rTransform.Position, _CollisionRadius, Color.cyan);
                }
#endif
            }
            ...
            if (!lIsColliding && mHadCollided)
            {
                ...
                {
                    rTransform.Position = lNewFocusPosition + (lNewDirection * (lOldDistance + (_CollisionRecoverySpeed * rDeltaTime)));
                }
                // No recovery needed
                else
                {
