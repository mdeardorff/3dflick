...
namespace com.ootii.Cameras
{
    ...
    public class OrbitFollowMotor : YawPitchMotor
    {
        ...
        public override CameraTransform RigLateUpdate(float rDeltaTime, int rUpdateIndex, float rTiltAngle = 0f)
        {
            ...
            mFrameEuler = GetFrameEuler(true);

            // Get our predicted based on the frame and anchor changes. We want this to get the right-vector
            Quaternion lNewCameraRotation = Quaternion.AngleAxis(mFrameEuler.y + lAnchorYaw, lTilt.Up()) * lCameraTransform.rotation;
            ...
            if (RigController.FrameForceToFollowAnchor || Mathf.Abs(rTiltAngle) >= 2f)
            {
                ...
                Matrix4x4 lOldFocusMatrix = Matrix4x4.TRS(mFocusLastPosition, mAnchorLastRotation, Vector3.one);
                Matrix4x4 lNewFocusMatrix = Matrix4x4.TRS(lNewFocusPosition, lAnchorTransform.rotation, Vector3.one);
            ...
            else
            {
                ...
                lNewCameraRotation = Quaternion.LookRotation(lToFocusPosition.normalized, lAnchorTransform.up);
                ...
                Vector3 lAnchorPosition = lAnchorTransform.position + (lAnchorTransform.rotation * AnchorOffset) + (lAnchorTransform.up * _Offset.y);
                ...
                Vector3 lToFocusPositionVertical = Vector3.Project(lToFocusPosition, lTilt.Up());
                ...
                Vector3 lRotationEuler = (Quaternion.Inverse(lTilt) * lNewCameraRotation).eulerAngles;

                // Add the pitch to our look-at rotation
                Quaternion lNewAnchorRotation = Quaternion.LookRotation(lToFocusPositionHorizontal, lTilt.Up()) * Quaternion.Euler(lRotationEuler.x, 0f, 0f);
            ...
            Quaternion lNewLocalCameraRotation = Quaternion.Inverse(Anchor.transform.rotation) * lNewCameraRotation;
            ...
            {
                lNewCameraRotation = Anchor.transform.rotation * Quaternion.Euler(lPitch, lYaw, 0f);
                lNewCameraPosition = lNewFocusPosition - (lNewCameraRotation.Forward() * mDistance);
            }
