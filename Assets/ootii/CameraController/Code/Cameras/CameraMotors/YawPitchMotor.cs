    ...
    public abstract class YawPitchMotor : CameraMotor
    {
        ...
        public virtual float LocalYaw
        {
            get
            {
                ...
                if (lAnchor == null) { return RigController._Transform.rotation.eulerAngles.y; }
        ...
        public virtual float LocalPitch
        {
            ...
            {
                Transform lAnchor = Anchor;
                if (lAnchor == null) { return RigController._Transform.rotation.eulerAngles.x; }
        ...
        /// <returns></returns>
        public virtual Vector3 GetFrameEuler(bool rUseYawLimits)
        {
            ...
            else if (mTargetYaw < float.MaxValue || mTargetPitch < float.MaxValue)
            {
                if (mTargetYaw < float.MaxValue)
                {
                    ...
                    if (Mathf.Abs(lFrameEuler.y) < EPSILON)
                    {
                        _Euler.y = mTargetYaw;
            ...
            else
            {
                ...
                lFrameEuler.y = (_Smoothing <= 0f ? _EulerTarget.y : Mathf.SmoothDampAngle(_Euler.y, _EulerTarget.y, ref mViewVelocityY, _Smoothing)) - _Euler.y;
                _Euler.y = _Euler.y + lFrameEuler.y;

                // Grab the smoothed pitch
                _EulerTarget.x = Mathf.Clamp(_EulerTarget.x + lFrameEuler.x, _MinPitch, _MaxPitch);
                lFrameEuler.x = (_Smoothing <= 0f ? _EulerTarget.x : Mathf.SmoothDampAngle(_Euler.x, _EulerTarget.x, ref mViewVelocityX, _Smoothing)) - _Euler.x;
